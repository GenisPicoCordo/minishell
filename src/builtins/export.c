...
/*   Updated: 2025/05/21 10:42:36 by gpico-co         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	validate_and_set_env_var(char *key, char *value, char *arg, \
	t_env **env_list)
{
	(void)arg;
	if (has_unclosed_quotes(value))
	{
...
int	process_single_export(char *arg, t_env **env_list)
{
	...
	if (has_equal)
		return (validate_and_set_env_var(key, value, arg, env_list));
	else
